...
BASE_DIRS = r'C:\Users\a7825\Desktop\工作空间\语音数据\PASD_out\第一次实验\文件名'
#主要文件的路径
...
def zidian(dir):#建立三个字典
    ...
    txtwenjian = csv.reader(open(os.path.join(BASE_DIRS, dir,name), 'r',encoding='utf-8'))#把symbol标志文件读进来,编码有问题的话就用encoding='utf-8'
    ...
    for per_dirs in os.listdir(BASE_DIRS):  # per_dirs = C001L,C001R...
        ...
        for id in os.listdir(os.path.join(BASE_DIRS,per_dirs,name_tezheng)):#id = C001L,C001R下面的文件的名字
            ...
            for i in range(start+1):
                t_file_list[i].insert(0, '9')#最前面的无音区间全部都打标签0,把它们当做正确认识来处理
            ...
            for y in dianout:#dianout是识别结果跟对应的帧数表
                ...
                if y[1][1]+1 <= len(t_file_list):#判断这个单词的范围是否超出了特征值得总行数
                    ...
                    if y[0] == dianout[-1][0]:
                        ...
                        #这段代码是为了把最后句号的部分全部打上标签9而设置的注意一下，下面也有一段代码
