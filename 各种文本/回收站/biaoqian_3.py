#第三版，给特征值打标签的文本,用于メルフィルタバンク、可以把前面的无音区间和后面的句号的部分全部都打上标签9，之后再全部删掉
import os
import data_3 as data_3#不要管这里的错误提醒，直接运行代码就行了
import data_4 as data_4
import re
import openpyxl
import csv
        ...
        for cor_dict in corresponding_dict[cl]:
            ...
            for index, item in enumerate(result_text.split('/')):#result_text是一句正解文，split返回的是一个列表
                ...
                if out_list[index][1][1]<=len(t_file_list):#关键在于要找到特征值的最后一行的编号落在识别结果的哪段单词的帧数的区间里
                    ...
                    t_file_list = data_3.biaoqian(out_list,item,t_file_list,index)

                else:

                    t_file_list = data_4.biaoqian(out_list,item,t_file_list,index)
                    break
