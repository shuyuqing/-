#用于比较正解文的单词数和识别结果的单词数是否一致
import os,dafa_1,dafa_2
import re
import openpyxl
import csv
    ...
    for cl in corresponding_dict:#迭代字典里面的东西跟迭代list里面的东西不一样
        ...
        for cor_dict in corresponding_dict[cl]:
            ...
            for index, item in enumerate(result_text.split('/')):#result_text是一句正解文，split返回的是一个列表
                ...
                #
                #     t_file_list = dafa_1.biaoqian(out_list,item,t_file_list,index)
                #
                # else:
                #
                #     t_file_list = dafa_2.biaoqian(out_list,item,t_file_list,index)
                #     break
