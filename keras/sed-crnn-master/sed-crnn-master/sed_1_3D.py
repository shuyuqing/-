...
from keras.layers import Bidirectional, TimeDistributed, Conv2D, MaxPooling2D, Input, GRU, Dense, Activation, Dropout, Reshape, Permute,Conv3D, MaxPooling3D
...
def get_model(data_in, data_out, _cnn_nb_filt, _cnn_pool_size, _rnn_nb, _fc_nb):
    ...
    for _i, _cnt in enumerate(_cnn_pool_size):#给列表里面的元素加上序号
        spec_x = Conv3D(filters=_cnn_nb_filt, kernel_size=(3, 3, 3), padding='same')(spec_x)
        spec_x = BatchNormalization(axis=1)(spec_x)
        spec_x = Activation('relu')(spec_x)
        spec_x = MaxPooling3D(pool_size=(1, _cnn_pool_size[_i]))(spec_x)
